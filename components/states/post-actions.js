...
export function StoreItemName(id, name) {
    ...
    return (dispatch, getState) => {
        ...
        return StoreItemNameFromApi(id, name).then(post => {
...
export function clearItems() {
    return (dispatch, getState) => {
        ...
        return clearItemFromApi().then(post => {
            dispatch(listRemindItem());
        }).catch(err => {
            console.error('Error list post', err);
        }).then(() => dispatch(endItemLoading()));
...
export function deleteRemindItem(id) {
    return (dispatch, getState) => {
        ...
        return deleteRemindItemFromApi(id).then(post => {
            dispatch(listRemindItem());
        }).catch(err => {
            console.error('Error list post', err);
        }).then(() => dispatch(endItemLoading()));
...
export function listRemindItem(searchText) {
    return (dispatch, getState) => {
        dispatch(startItemLoading());
        return listRemindItemFromApi(searchText).then(items => {
...
export function createSet(id, setting) {
    return (dispatch, getState) => {
        ...
        return SetRemindItemFromApi(id, setting).then(post => {
            dispatch(listRemindItem());
        }).catch(err => {
            console.error('Error list post', err);
        }).then(() => dispatch(endItemLoading()));
...
export function createRemindItem(id, setting) {
    return (dispatch, getState) => {
        ...
        return createRemindItemFromApi(id, setting).then(item => {
            dispatch(listRemindItem());
        }).catch(err => {
...
export function listMorePosts(searchText) {
    return (dispatch, getState) => {
        dispatch(startLoading());
        return listPostsFromApi(searchText, state.posts[state.posts.length - 1].id).then(posts => {
...
export function createCheck(id) {
    return (dispatch, getState) => {
        ...
        return createCheckFromApi(id).then(post => {
            dispatch(listPosts());
        }).catch(err => {
            console.error('Error creating vote', err);
        }).then(() => dispatch(endLoading()));
